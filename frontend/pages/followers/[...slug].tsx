...
const MintPage: NextPage = () => {
  ...
  const {write, isLoading, isSuccess} = useMint(influencer ?? '0x00', campaign ?? 0)
  ...
  return (
          <div className={styles.mint}>
            <div className={styles.dialog}>
              ...
              <div style={{display: verified ? 'none': 'flex'}} className='flex-col mb-4 items-center justify-center text-center'>
                  <div className='mb-4'>
                      First verify you are a human
                  </div>
                  <VerifyWorldId verified={verified} setVerified={setVerified} />
              </div>
              <button className={styles.btn} style={{display: verified ? 'flex': 'none'}} onClick={()=>{
                if(influencer!==null && campaign !== null && write){
                  write()
                }
              }}>{influencer==null  || campaign == null || isLoading ? 'Loading' : 'Mint NFT'}</button>
            </div>
          </div>
  );
};
